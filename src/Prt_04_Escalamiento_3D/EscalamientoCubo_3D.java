...
package Prt_04_Escalamiento_3D;
...
public class EscalamientoCubo_3D extends JPanel implements KeyListener {

    private BufferedImage buffer;
    private double[][] vertices;
    private int[][] edges;
    private final double escala = 40;

    private final double[][] puntoCubo = {{350, 250, 0}};
    private final double[][] puntoFuga = {{200, 250, 500}};

    public EscalamientoCubo_3D() {
    ...
    public static JFrame generarFrame() {
        ...
        frame.setTitle("Cubo 3D con Escalamiento");
        frame.setSize(700, 580);
        frame.setResizable(false);
        frame.setLocationRelativeTo(null);
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        frame.add(new EscalamientoCubo_3D());
        return frame;
    }
    ...
    private void escalar(double factor) {
        for (int i = 0; i < vertices.length; i++) {
            vertices[i][0] *= factor; // Escalado en X
            vertices[i][1] *= factor; // Escalado en Y
            vertices[i][2] *= factor; // Escalado en Z
        }
        repaint();
    }
