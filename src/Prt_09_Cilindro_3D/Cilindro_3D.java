...
public class Cilindro_3D extends JPanel implements KeyListener, Runnable {
    ...
    private final double[] puntoFuga = { 0, 0, 1000 };
    ...
    private void dibujarCilindro(Graphics g) {
        int pasos = 100;
        double t_min = -2 * Math.PI;
        ...
        for (int i = 0; i < pasos; i++) {
            for (int j = 0; j < pasos; j++) {
                ...
                double radio = 1 + Math.abs(Math.cos(t1));
                double x1 = radio * Math.cos(theta1) * escala;
                double y1 = radio * Math.sin(theta1) * escala;
                double z1 = t1 * escala / Math.PI;
                
                double t2 = t_min + (i + 1) * (t_max - t_min) / pasos;
                double theta2 = theta_min + j * (theta_max - theta_min) / pasos;
                
                double radio2 = 1 + Math.abs(Math.cos(t2));
                double x2 = radio2 * Math.cos(theta2) * escala;
                double y2 = radio2 * Math.sin(theta2) * escala;
                double z2 = t2 * escala / Math.PI;
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(Cilindro_3D::new);
    }
